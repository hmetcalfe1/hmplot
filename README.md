...
This repo contains the functions of the `hmplot` package, which once installed locally, provides helpful functions for creating and exporting  graphics made in ggplot in the style used by the hm News data team.

![Example of graphics created using the hmplot package](chart_examples/hmplot_example_plots.png)

## Installing hmplot

`hmplot` is not on CRAN, so you will have to install it directly from Github using `devtools`. 
...
Example of how the `finalise_plot()` is used in a standard workflow. This function is called once you have created and finalised your chart data, titles and added the `hm_style()` to it (see above):
